"""Seedance 2.5 via the Higgsfield SDK. Credentials: HF_KEY in .env.local (never printed).
    ...
    .venv/bin/python tools/seedance.py "prompt" OUT.mp4 [--duration 5] [--res 720p] [--aspect 16:9] [--no-audio]
...
def generate(prompt, out=None, duration=5, resolution='720p', aspect='16:9', audio=True):
...
if __name__ == '__main__':
    ...
    if not a:
        ...
        generate(a[0], a[1] if len(a) > 1 and not a[1].startswith('--') else None, int(opt('--duration', 5)), opt('--res', '720p'), opt('--aspect', '16:9'), '--no-audio' not in a)
