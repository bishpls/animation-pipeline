...
(() => {
  ...
  function s02_block(t, lt, dur) {
    ...
    const cy = lerp(930, 60, tilt), cx = lerp(BLDG.x + 70 + BLDG.w * .275, kx + kw / 2, E.io2(clamp(lt / 1.6)) * .3 + tilt * .7);
    const zoom = lerp(2.2, 1.0, E.io3(clamp(lt / (dur - .9)))) * lerp(1, 7.5, push);
    ...
    ink(P(cut(rect(-400, 620, 900, 900), 21)), 'black');
    ink(P(cut(rect(1420, 180, 900, 1100), 22)), 'black');
    for (let i = 0; i < 14; i++) { const x = -330 + (i % 7) * 115, y = 680 + Math.floor(i / 7) * 150; if (hash(i + 3) > .55) { knock(P(rect(x, y, 50, 80)), ['black']); if (hash(i + 9) > .5) { knock(P(rect(x, y, 50, 80)), ['blue']); ink(P(rect(x, y, 50, 80)), { yellow: 1 }); } } }
    ...
    paint(P(cut(rect(-600, 1078, 3200, 700), 23)), 'black');                        // the street
    knock(P([[-600, 1112], [2600, 1112]], false), ['black'], 1, { stroke: 5, dash: [60, 40] });
  ...
  PROJECT.subtitleAt = t => (t > 6.1 && t < 7.3);
})();
