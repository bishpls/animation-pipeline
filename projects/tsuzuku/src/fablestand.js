...
{
  const B = 60 / 170 * 4, beat = B / 2, S = 130, LEG = 1280, SC = .22;   // stride S (canvas px); leg = hip to ankle (master px)
  // three steps, one per beat: the visible foot, the hidden foot, the visible foot again (ending feet together)
  const STEPS = [{ t: 3 * beat, foot: 'v' }, { t: 4 * beat, foot: 'h' }, { t: 5 * beat, foot: 'v', close: true }];
  ...
  const walk = tq => {
    let k = STEPS.findIndex((st, i) => tq < st.t + beat && tq >= st.t), u = 0;
    let feet;
    if (k < 0) { const done = STEPS.filter(st => tq >= st.t + beat).length; feet = plan[done]; }
    else {
      u = (tq - STEPS[k].t) / beat; const a = plan[k], b = plan[k + 1], e = ease(u);
    ...
    p.upperarm = 5 * Math.sin(Math.PI * 2 * Math.min(1, (tq - STEPS[0].t) / (3 * beat))) * (tq > STEPS[0].t ? 1 : 0);   // arm counter-swing
